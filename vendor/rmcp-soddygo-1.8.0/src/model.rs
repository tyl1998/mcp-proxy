...
pub struct CallToolResult {
    ...
    #[serde(rename = "_meta", skip_serializing_if = "Option::is_none")]
    pub meta: Option<Meta>,
    /// Multi-round tool result type (protocol revision 2026-07-28, MRTR):
    /// `complete` or `input_required`. The 2026 wire codec puts `resultType`
    /// at the **result top level** — NOT inside `structuredContent`. Local
    /// patch: without this field the serde default (ignore unknown fields)
    /// silently drops the MRTR discriminator while proxying, so clients only
    /// ever see `content: []` and the confirmation round never starts.
    #[serde(
        rename = "resultType",
        default,
        skip_serializing_if = "Option::is_none"
    )]
    pub result_type: Option<Cow<'static, str>>,
    /// MRTR embedded input requests (`input_required` round 1). Top-level
    /// reserved key on the result wire shape. Local patch (see above).
    #[serde(
    ...
    /// MRTR opaque round-correlation token echoed back on the retry round.
    /// Local patch (see above).
    #[serde(
        rename = "requestState",
        default,
        skip_serializing_if = "Option::is_none"
...
impl<'de> Deserialize<'de> for CallToolResult {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct Helper {
            ...
            result_type: Option<Cow<'static, str>>,
        ...
        if helper.content.is_none()
            ...
            && helper.input_requests.is_none()
            && helper.request_state.is_none()
        {
...
pub struct CallToolRequestParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub task: Option<TaskMetadata>,
    /// Multi-round tool result user responses (protocol revision 2026-07-28,
    /// MRTR). This is a params-**top-level** reserved key (the TS SDK's
    /// `RETRY_PARAMS_KEYS`) and must NOT travel inside `_meta` — the server
    /// lifts it from the top level only. Local patch: without this field the
    /// serde default (ignore unknown fields) silently drops the second-round
    /// confirmation payload when proxying, degrading the confirm round into a
    /// new first round. See vendor README.patch.md.
    #[serde(
...
impl CallToolRequestParams {
    ...
    /// Sets the MRTR input responses for this tool call (protocol revision
    /// 2026-07-28, second round).
    pub fn with_input_responses(mut self, input_responses: serde_json::Value) -> Self {
